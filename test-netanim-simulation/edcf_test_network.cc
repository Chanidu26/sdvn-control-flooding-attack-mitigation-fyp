...
/* Compute HMAC_{k_g}(m_i || ts_i || id_i)  -- proposal equation 3.2
   m_i  = message content: pkt_type(4B) + fake_x(8B) + fake_y(8B)
                         + real_x(8B)  + real_y(8B)  = 36 bytes
   ts_i = monotonic timestamp in nanoseconds (8 bytes, anti-replay)
   id_i = vehicle pseudonym = src_node_id (4 bytes)
   k_g  = g_net_key (shared group key, distributed to all legit nodes)
   Total message input = 36 + 8 + 4 = 48 bytes */
                          ...
                          uint8_t digest[32])
{
    uint8_t msg[48];
    /* m_i: message content */
    memcpy(msg+0,  &pkt_type, 4);
    memcpy(msg+4,  &fake_x,   8);
    memcpy(msg+12, &fake_y,   8);
    memcpy(msg+20, &real_x,   8);
    memcpy(msg+28, &real_y,   8);
    /* ts_i: monotonic timestamp (anti-replay) */
    memcpy(msg+36, &timestamp, 8);
    /* id_i: vehicle pseudonym */
    memcpy(msg+44, &src_node,  4);
    hmac_sha256(sender_key(src_node), 32, msg, 48, digest);
}

/* Verify HMAC_{k_g}(m_i || ts_i || id_i) -- proposal equation 3.2
   Always verify using g_net_key (shared group key).
   Legitimate senders used g_net_key -> MATCH -> TP
   External ATK used g_atk_key      -> MISMATCH -> TN (detected)
   Stolen key ATK uses g_net_key    -> MATCH -> FN (evades HMAC) */
                         ...
                         const uint8_t recv_digest[32])
{
    ...
    memcpy(msg+28, &real_y,      8);
    memcpy(msg+36, &timestamp,   8);
    memcpy(msg+44, &sender_node, 4);
    /* Always verify with network key -- ATK without stolen key fails here */
    hmac_sha256(g_net_key, 32, msg, 48, expected);
...
public:
    ...
    uint32_t GetSerializedSize() const override {
        return 2*sizeof(uint32_t) + 9*sizeof(double) + sizeof(uint64_t) + 32;
    }
    void Serialize(TagBuffer buf) const override {
        ...
        buf.Write((const uint8_t*)&m_timestamp,  sizeof(uint64_t));
        buf.Write(m_hmac, 32);
    }
    void Deserialize(TagBuffer buf) override {
        ...
        buf.Read((uint8_t*)&m_timestamp,  sizeof(uint64_t));
        buf.Read(m_hmac, 32);
    }
    ...
    uint64_t m_timestamp=0;  /* ts_i: monotonic sim time in ns (proposal eq 3.2) */
    uint8_t m_hmac[32]={0};
};
...
/* ================================================================
   RULE-BASED SIGNATURE DETECTION  (proposal Section 3.2.4)
   Equations 3.9-3.18: three signature scores PHI_V1, PHI_V2, PHI_V3
   Tracked per-cycle, computed at print_pem time.
   ================================================================ */

/* V1 Signature inputs (eq 3.9-3.12):
   beacon_count_p: beacons received from all sources in this cycle
   novel_id_count_p: beacons from IDs not seen in last Tmem window
   flowmod_count_p: FlowMod messages received (table-miss proxy)
   legit_beacon_p:  legitimate beacons (denominator for rate) */
static uint32_t g_beacon_count_p=0,  g_beacon_count=0;
static uint32_t g_novel_id_count_p=0, g_novel_id_count=0;
static uint32_t g_flowmod_count_p=0,  g_flowmod_count=0;
static uint32_t g_legit_beacon_p=0,   g_legit_beacon=0;

/* V2 Signature inputs (eq 3.13-3.15):
   alert_out_p: CASCADE/alert pkts forwarded (Aout)
   alert_in_p:  FAKE_ALERT pkts received (Ain)
   recompute_p: RECOMPUTE pkts = control-plane load indicator */
static uint32_t g_alert_out_p=0,  g_alert_out=0;
static uint32_t g_alert_in_p=0,   g_alert_in=0;
static uint32_t g_recompute_p=0,  g_recompute=0;

/* V3 Signature inputs (eq 3.16-3.18):
   route_chg_p: ROUTE_CHG pkts (V2->C1, triggered by fake trace)
   wrong_topo_p: WRONG_TOPO pkts (topology inconsistency indicator)
   pos_jump_p: fake positions with large displacement from real pos */
static uint32_t g_route_chg_p=0,  g_route_chg=0;
static uint32_t g_wrong_topo_p=0, g_wrong_topo=0;
static uint32_t g_pos_jump_p=0,   g_pos_jump=0;

/* Signature score thresholds (from proposal):
   Theta_V1 in (0,3], Theta_V2 in (0,3], Theta_V3 in (0,3] */
static const double THETA_V1 = 1.5;  /* flag V1 if PHI_V1 >= 1.5 */
static const double THETA_V2 = 1.5;  /* flag V2 if PHI_V2 >= 1.5 */
static const double THETA_V3 = 1.5;  /* flag V3 if PHI_V3 >= 1.5 */
static const double ALPHA1   = 2.0;  /* z-score threshold for beacon rate */
static const double ALPHA2   = 2.0;  /* z-score threshold for table-miss */
static const double PHI_TH   = 2.0;  /* fan-out ratio threshold */
static const double PSI_TH   = 0.1;  /* inter-alert dispersion threshold */
static const double TAU_TH   = 0.7;  /* directional consistency threshold */
static const double ZETA_TH  = 50.0; /* position jump threshold (meters) */
...
static EdcfTag make_tag(uint32_t src, EdcfTag::MsgType type) {
    ...
    /* ts_i: monotonic simulation timestamp in nanoseconds (anti-replay) */
    t.m_timestamp = Simulator::Now().GetNanoSeconds();
    /* Compute HMAC_{k_g}(m_i || ts_i || id_i) -- proposal eq 3.2 */
    compute_hmac(src, (uint32_t)type,
                 t.m_fake_x, t.m_fake_y,
                 t.m_real_x, t.m_real_y,
                 t.m_timestamp, t.m_hmac);
    g_tseq++; return t;
}
                             ...
                             EdcfTag::MsgType type, bool result, double t)
{
    ...
    if (!g_hmac_hdr){
        ...
        f << "# HMAC = HMAC_{k_g}(m_i||ts_i||id_i) per proposal eq 3.2\n";
        f << "# m_i=pkt_type+fake_xy+real_xy(36B) ts_i=ns_timestamp(8B) id_i=pseudonym(4B)\n";
        f << "t,sender,receiver,pkt_type,hmac_valid,result\n";
    ...
    f << fixed << setprecision(4) << t << ","
      ...
      << TNAME[(int)type] << ","
      << (result ? "YES" : "NO") << ","
      << ((!result && is_attack_type(type)) ? "DETECTED(TN)"
          : (!result && !is_attack_type(type)) ? "FALSE_ALARM(FP)"
          : (result && !is_attack_type(type)) ? "LEGIT(TP)"
          : "MISSED(FN)")
      << "\n";
    f.close();
}
...
static void PktRxCb(uint32_t rid, Ptr<const Packet> pkt){
    ...
    /* =================================================================
     * LIGHTWEIGHT MODE -- proposal Section 3.2.2 + 3.2.4
     *
     * Controllers (C0,C1,C2) act as RSU+controller in our topology.
     * Detection order matches proposal exactly:
     *
     *   STEP 1: Rule-based signature score (fast O(1) pre-filter)
     *           -- computed from observable network quantities
     *           -- if score signals attack -> DROP immediately
     *   STEP 2: HMAC verification (eq 3.2) -- only if Step 1 passes
     *           -- if HMAC fails -> DROP
     *
     * "Events whose HMAC verification fails OR whose signature score
     *  exceeds a threshold are dropped" -- proposal Section 3.2.2
     * ================================================================ */

    /* Controllers act as RSU -- apply both checks.
       Vehicles only receive V2V: apply HMAC only (no RSU role). */
    bool at_controller = (rid==IDX_C0 || rid==IDX_C1 || rid==IDX_C2);

    /* ---- STEP 1: RULE-BASED SIGNATURE PRE-FILTER (Section 3.2.4) ----
       Per-packet instant signature check using running window counters.
       Full PHI_V1/V2/V3 scores are computed per-cycle in print_pem.
       Here: fast threshold check on current window state. */
    bool sig_flag  = false;
    string sig_status = "SIG:OK";

    if (at_controller) {
        /* V1 check (eq 3.9-3.12): beacon rate + novel ID + table-miss */
        if (tag.m_type==EdcfTag::FAKE_BEACON) {
            double rho = (double)g_novel_id_count_p /
                         (double)max((uint32_t)1u, g_beacon_count_p+1);
            if (rho > 0.3) {        /* >30% novel IDs = suspicious (rho_new) */
                sig_flag = true;
                sig_status = "SIG:V1_NOVEL_ID_RATE(pre-HMAC)";
            }
        }
        if (tag.m_type==EdcfTag::WRONG_FM) {
            double xi = (double)g_flowmod_count_p / 3.5;
            if (xi > ALPHA2) {      /* FlowMod surge = table-miss indicator */
                sig_flag = true;
                sig_status = "SIG:V1_FLOWMOD_SURGE(pre-HMAC)";
            }
        }
        /* V2 check (eq 3.13-3.15): fan-out ratio + control-plane load */
        if (tag.m_type==EdcfTag::FAKE_ALERT || tag.m_type==EdcfTag::CASCADE) {
            double phi_i = (double)g_alert_out_p /
                           (double)max((uint32_t)1u, g_alert_in_p+1);
            if (phi_i > PHI_TH) {   /* fan-out ratio too high */
                sig_flag = true;
                sig_status = "SIG:V2_FANOUT_RATIO(pre-HMAC)";
            }
            double lambda_cp = (double)g_recompute_p / 3.5;
            if (lambda_cp > 2.0) {  /* RECOMPUTE flood = control-plane surge */
                sig_flag = true;
                sig_status = "SIG:V2_CP_LOAD(pre-HMAC)";
            }
        }
        /* V3 check (eq 3.17-3.18): position jump + WRONG_TOPO rate */
        if (tag.m_type==EdcfTag::FAKE_TRACE || tag.m_type==EdcfTag::ROUTE_CHG) {
            double dx = tag.m_fake_x - tag.m_real_x;
            double dy = tag.m_fake_y - tag.m_real_y;
            if (sqrt(dx*dx+dy*dy) > ZETA_TH) {  /* position jump > 50m */
                sig_flag = true;
                sig_status = "SIG:V3_POS_JUMP(pre-HMAC)";
            }
        }
        if (tag.m_type==EdcfTag::WRONG_TOPO) {
            double lm = (double)g_wrong_topo_p / 3.5;
            if (lm > 1.0) {         /* WRONG_TOPO rate surge */
                sig_flag = true;
                sig_status = "SIG:V3_TOPO_RATE(pre-HMAC)";
            }
        }
    }

    /* ---- STEP 2: HMAC AUTHENTICATION (eq 3.2) ----
       Only reached if signature pre-filter did NOT flag the packet.
       If signature already flagged: skip HMAC (packet already dropped). */
    bool hmac_ok = false;
    g_hmac_verified++; g_hmac_verified_p++;
    if (!sig_flag) {
        hmac_ok = verify_hmac(src, (uint32_t)tag.m_type,
                              tag.m_fake_x, tag.m_fake_y,
                              tag.m_real_x, tag.m_real_y,
                              tag.m_timestamp, tag.m_hmac);
        if (!hmac_ok) { g_hmac_failed++; g_hmac_failed_p++; }
    } else {
        /* Signature pre-filter already dropped packet.
           Count HMAC as failed (packet dropped at pre-filter stage). */
    ...
    string hmac_status;
    if (sig_flag && atk) {
        /* Attack caught by signature BEFORE HMAC -> TN
           This is the key advantage: signature catches insider/stolen-key
           attacks that HMAC alone cannot detect */
        hmac_status = sig_status + " -> BLOCKED_PRE_HMAC(TN)";
        g_TN++; g_TN_p++;
    } else if (sig_flag && !atk) {
        /* Legit wrongly flagged by signature -> FP (false alarm) */
        hmac_status = sig_status + " -> FALSE_ALARM(FP)";
        g_FP++; g_FP_p++;
        g_legit_total++; g_legit_total_p++;
        g_legit_drop++;  g_legit_drop_p++;
    } else if (!hmac_ok && atk) {
        /* Passed signature but HMAC caught it -> TN */
        hmac_status = "SIG:OK + HMAC:FAIL -> BLOCKED_BY_HMAC(TN)";
        g_TN++; g_TN_p++;
    } else if (!hmac_ok && !atk) {
        /* Legit failed HMAC -> FP */
        hmac_status = "SIG:OK + HMAC:FAIL -> LEGIT_BLOCKED(FP)";
        g_FP++; g_FP_p++;
        g_legit_total++; g_legit_total_p++;
        g_legit_drop++;  g_legit_drop_p++;
    } else if (hmac_ok && !atk) {
        /* Both pass, legitimate packet -> TP */
        hmac_status = "SIG:OK + HMAC:OK -> ACCEPTED(TP)";
        g_TP++; g_TP_p++;
        g_legit_total++; g_legit_total_p++;
        if (rid < 5) {
        ...
        /* Both pass but IS attack (sophisticated: evades sig + has valid key)
           -> FN: lightweight mode insufficient -> FULL MODE needed */
        hmac_status = "SIG:OK + HMAC:OK_BUT_ATTACK -> MISSED(FN) -- FULL_MODE_NEEDED";
        g_FN++; g_FN_p++;
    }

    /* Impact counters (channel flooding, wrong FM/topo counts) */
    if (atk) {
        if (tag.m_type==EdcfTag::FAKE_BEACON ||
    ...
    /* ---- SIGNATURE INPUT TRACKING (proposal eq 3.9-3.18) ---- */
    /* V1 inputs */
    if (tag.m_type==EdcfTag::BEACON || tag.m_type==EdcfTag::FAKE_BEACON) {
        g_beacon_count++; g_beacon_count_p++;
        if (tag.m_type==EdcfTag::BEACON)
            { g_legit_beacon++; g_legit_beacon_p++; }
        else
            /* Novel ID: attacker uses spoofed ID not in blockchain */
            { g_novel_id_count++; g_novel_id_count_p++; }
    }
    if (tag.m_type==EdcfTag::WRONG_FM || tag.m_type==EdcfTag::FLOWMOD)
        { g_flowmod_count++; g_flowmod_count_p++; }
    /* V2 inputs */
    if (tag.m_type==EdcfTag::FAKE_ALERT)
        { g_alert_in++; g_alert_in_p++; }
    if (tag.m_type==EdcfTag::CASCADE)
        { g_alert_out++; g_alert_out_p++; }
    if (tag.m_type==EdcfTag::RECOMPUTE)
        { g_recompute++; g_recompute_p++; }
    /* V3 inputs */
    if (tag.m_type==EdcfTag::ROUTE_CHG)
        { g_route_chg++; g_route_chg_p++; }
    if (tag.m_type==EdcfTag::WRONG_TOPO)
        { g_wrong_topo++; g_wrong_topo_p++; }
    /* Position jump: large displacement between fake and real position
       zeta_i = |fake_pos - real_pos| -- eq 3.17 proxy */
    if (atk && (tag.m_type==EdcfTag::FAKE_BEACON ||
                tag.m_type==EdcfTag::FAKE_TRACE)) {
        double dx = tag.m_fake_x - tag.m_real_x;
        double dy = tag.m_fake_y - tag.m_real_y;
        double dist = sqrt(dx*dx + dy*dy);
        if (dist > ZETA_TH)
            { g_pos_jump++; g_pos_jump_p++; }
    }
...
/* ================================================================
   ...
   ================================================================ */
/* ================================================================
   COMPUTE SIGNATURE SCORES per proposal Section 3.2.4
   Returns PHI_V1, PHI_V2, PHI_V3 and detection flags.
   Uses per-cycle (_p) or cumulative counters based on caller.
   ================================================================ */
struct SigScores {
    double phi_v1, phi_v2, phi_v3;
    bool   flag_v1, flag_v2, flag_v3;
    /* Component values for logging */
    double Si, rho_new, xi_tm;   /* V1 components */
    double phi_i, psi, lambda_cp; /* V2 components */
    double tau_dir, zeta_i, lambda_fm; /* V3 components */
};

static SigScores compute_signatures(
    uint32_t beacon_cnt, uint32_t novel_id, uint32_t flowmod_cnt,
    uint32_t legit_bcn,
    uint32_t alert_out, uint32_t alert_in, uint32_t recompute_cnt,
    uint32_t route_chg, uint32_t wrong_topo, uint32_t pos_jump,
    double window_s)
{
    SigScores s; memset(&s, 0, sizeof(s));

    /* ---- V1: Spoofed Beacon Flooding (eq 3.9-3.12) ---- */
    /* Si(t) = (ri - r_mean) / sigma_r  -- beacon rate z-score
       Proxy: attack beacons / max(legit beacons, 1) as rate ratio */
    double r_atk  = (double)(beacon_cnt - legit_bcn);  /* fake beacons */
    double r_legit = (double)max((uint32_t)1u, legit_bcn);
    s.Si      = r_atk / r_legit;                    /* rate anomaly (eq 3.9 proxy) */
    s.rho_new = beacon_cnt>0 ? (double)novel_id/beacon_cnt : 0.0; /* identity novelty (eq 3.10) */
    s.xi_tm   = window_s>0 ? (double)flowmod_cnt/window_s : 0.0;  /* table-miss rate (eq 3.11 proxy) */
    /* PHI_V1 = (1/|V|) sum [ I(Si>alpha1) + rho_new + I(xi_tm>alpha2) ] (eq 3.12) */
    s.phi_v1  = (s.Si > ALPHA1 ? 1.0 : 0.0)
              + s.rho_new
              + (s.xi_tm > ALPHA2 ? 1.0 : 0.0);
    s.flag_v1 = (s.phi_v1 >= THETA_V1);

    /* ---- V2: Cascading Alert Propagation (eq 3.13-3.15) ---- */
    /* phi_i = Aout / max(Ain, 1) -- alert fan-out ratio (eq 3.13) */
    s.phi_i    = (double)alert_out / (double)max((uint32_t)1u, alert_in);
    /* psi: inter-alert interval dispersion (eq 3.14)
       Proxy: low psi = coordinated injections
       We model as 1/(alert_in+1) -- more alerts = lower psi = more coordinated */
    s.psi      = 1.0 / (double)(alert_in + 1);
    /* lambda_cp: control-plane load = RECOMPUTE rate (eq 3.15 proxy) */
    s.lambda_cp = window_s>0 ? (double)recompute_cnt/window_s : 0.0;
    /* PHI_V2 = I(phi_i > phi_th) + I(psi < psi_th) + I(lambda_cp > mean+alpha*sigma)
       Simplified: I(phi_i>PHI_TH) + I(psi<PSI_TH) + I(lambda_cp>2.0) */
    s.phi_v2   = (s.phi_i > PHI_TH ? 1.0 : 0.0)
               + (s.psi < PSI_TH ? 1.0 : 0.0)
               + (s.lambda_cp > 2.0 ? 1.0 : 0.0);
    s.flag_v2  = (s.phi_v2 >= THETA_V2);

    /* ---- V3: Mobility Trace Manipulation (eq 3.16-3.18) ---- */
    /* tau_dir: directional consistency index (eq 3.16)
       Proxy: 1 - pos_jump/max(route_chg,1)
       Many position jumps -> low tau_dir */
    s.tau_dir  = 1.0 - (double)pos_jump / (double)max((uint32_t)1u, route_chg + wrong_topo);
    if (s.tau_dir < 0.0) s.tau_dir = 0.0;
    /* zeta_i: trajectory-topology inconsistency (eq 3.17 proxy)
       pos_jump_p counts packets with |fake_pos - real_pos| > ZETA_TH */
    s.zeta_i   = (double)pos_jump;
    /* lambda_fm: FlowMod rate triggered by wrong topology (eq 3.18 proxy) */
    s.lambda_fm = window_s>0 ? (double)wrong_topo/window_s : 0.0;
    /* PHI_V3 = I(tau_dir < tau_th) + I(zeta_i > zeta_th) + I(lambda_fm > mean+alpha*sigma) */
    s.phi_v3   = (s.tau_dir < TAU_TH ? 1.0 : 0.0)
               + (s.zeta_i > 0 ? 1.0 : 0.0)
               + (s.lambda_fm > 1.0 ? 1.0 : 0.0);
    s.flag_v3  = (s.phi_v3 >= THETA_V3);
...
static void print_pem(){
    ...
    /* ---- SIGNATURE SCORES (proposal Section 3.2.4) ---- */
    SigScores sig_p = compute_signatures(
        g_beacon_count_p, g_novel_id_count_p, g_flowmod_count_p,
        g_legit_beacon_p,
        g_alert_out_p, g_alert_in_p, g_recompute_p,
        g_route_chg_p, g_wrong_topo_p, g_pos_jump_p,
        3.5);  /* window = 3.5s (cycle duration) */
    SigScores sig_c = compute_signatures(
        g_beacon_count, g_novel_id_count, g_flowmod_count,
        g_legit_beacon,
        g_alert_out, g_alert_in, g_recompute,
        g_route_chg, g_wrong_topo, g_pos_jump,
        t_now);

    cout << "\n  [SIGNATURE SCORES -- proposal Section 3.2.4]\n";
    cout << "  CYCLE:  PHI_V1=" << fixed << setprecision(3) << sig_p.phi_v1
         << (sig_p.flag_v1 ? " [V1 FLAGGED]" : " [V1 ok]");
    cout << "  PHI_V2=" << sig_p.phi_v2
         << (sig_p.flag_v2 ? " [V2 FLAGGED]" : " [V2 ok]");
    cout << "  PHI_V3=" << sig_p.phi_v3
         << (sig_p.flag_v3 ? " [V3 FLAGGED]" : " [V3 ok]") << "\n";
    cout << "  Si=" << sig_p.Si
         << " rho_new=" << sig_p.rho_new
         << " xi_tm=" << sig_p.xi_tm
         << " | phi_i=" << sig_p.phi_i
         << " psi=" << sig_p.psi
         << " | tau_dir=" << sig_p.tau_dir
         << " zeta=" << sig_p.zeta_i << "\n";

    /* Lightweight mode combined decision: HMAC OR signature score */
    cout << "  Lightweight Mode (HMAC + rule-based): ";
    if (sig_p.flag_v1 || sig_p.flag_v2 || sig_p.flag_v3)
        cout << "ATTACK FLAGGED by signature score\n";
    else if (g_hmac_failed_p > 0)
        cout << "ATTACK FLAGGED by HMAC\n";
    else
        cout << "No flag raised (insider/stolen key evades both)\n";

    /* Write signature scores to CSV */
    fstream fsig;
    string sig_csv = g_pem_csv.substr(0, g_pem_csv.size()-8) + "_sig.csv";
    static bool sig_hdr = false;
    if (!sig_hdr) {
        ifstream chk(sig_csv); bool fe=chk.good(); chk.close();
        if (!fe) {
            fsig.open(sig_csv, ios::out|ios::trunc);
            fsig << "# Signature Scores per proposal Section 3.2.4\n";
            fsig << "# PHI_V1(eq3.12) PHI_V2(eq3.15) PHI_V3(eq3.18)\n";
            fsig << "# Threshold: THETA=1.5 for all variants\n";
            fsig << "atk_count,atk_pct,cycle,row_type,t,";
            fsig << "PHI_V1,flag_V1,Si,rho_new,xi_tm,";
            fsig << "PHI_V2,flag_V2,phi_i,psi,lambda_cp,";
            fsig << "PHI_V3,flag_V3,tau_dir,zeta_i,lambda_fm,";
            fsig << "lw_mode_detected\n";
            fsig.close();
        }
        sig_hdr = true;
    }
    double _ap = 100.0*g_atk_count/(double)g_total_nodes;
    auto write_sig_row = [&](const SigScores& sg, const string& rt) {
        bool lw = sg.flag_v1 || sg.flag_v2 || sg.flag_v3;
        fsig.open(sig_csv, ios::out|ios::app);
        fsig << g_atk_count << "," << fixed << setprecision(1) << _ap << ",";
        fsig << g_pem_cycle << "," << rt << "," << setprecision(2) << t_now << ",";
        fsig << setprecision(4)
             << sg.phi_v1 << "," << sg.flag_v1 << ","
             << sg.Si << "," << sg.rho_new << "," << sg.xi_tm << ",";
        fsig << sg.phi_v2 << "," << sg.flag_v2 << ","
             << sg.phi_i << "," << sg.psi << "," << sg.lambda_cp << ",";
        fsig << sg.phi_v3 << "," << sg.flag_v3 << ","
             << sg.tau_dir << "," << sg.zeta_i << "," << sg.lambda_fm << ",";
        fsig << lw << "\n";
        fsig.close();
    };
    write_sig_row(sig_p, "CYCLE");
    write_sig_row(sig_c, "CUMULATIVE");
    cout << "[SIG CSV] -> " << sig_csv << "\n";
    ...
    /* Reset signature input counters */
    g_beacon_count_p=0; g_novel_id_count_p=0; g_flowmod_count_p=0;
    g_legit_beacon_p=0;
    g_alert_out_p=0; g_alert_in_p=0; g_recompute_p=0;
    g_route_chg_p=0; g_wrong_topo_p=0; g_pos_jump_p=0;
}
...
int main(int argc,char* argv[]){
    ...
    cout<<"\n=== EDCF-Shield  Group 14 (HMAC-SHA256 Enabled) ===\n"
        ...
        << "HMAC: HMAC_{k_g}(m_i||ts_i||id_i) -- proposal eq 3.2\n"
        << "  m_i=pkt_type+fake_xy+real_xy  ts_i=ns_timestamp  id_i=pseudonym\n"
        << "  k_g=g_net_key (256-bit shared group key, distributed to all legit nodes)\n"
        << "  ATK key mode: "
        << (g_atk_has_valid_key
